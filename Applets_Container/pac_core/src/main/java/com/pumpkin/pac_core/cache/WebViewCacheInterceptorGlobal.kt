...
class WebViewCacheInterceptorGlobal(
    ...
    class Builder {
        ...
        var dns: Dns? = null
            private set
        var cacheExtensionConfig: CacheExtensionConfig = CacheExtensionConfig()
        ...
        fun build(context: Context): WebViewCacheInterceptorGlobal {
    ...
    internal fun interceptRequest(request: WebResourceRequest?): WebResourceResponse? {
        if (request != null) {
    ...
    internal fun interceptRequest(url: String?) = interceptRequest(
        url, mapOf(
            "Origin" to mOrigin,
            "Referer" to mReferer,
    ...
    internal fun loadUrl(webView: WebView, url: String) {
        if (!isValidUrl(url)) {
            return
        }
        webView.loadUrl(url)
    ...
    internal fun enableCache(enable: Boolean) {
        cacheType = if (enable) {
            CacheType.CACHE
        } else {
            CacheType.NOCACHE
