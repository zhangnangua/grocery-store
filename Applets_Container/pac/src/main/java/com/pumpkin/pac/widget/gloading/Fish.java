...
public class Fish {
    ...
    //鱼身长度
    public final float BODY_LENGTH = HEAD_RADIUS * 3.2f;
    //寻找鱼鳍的初始点
    ...
    public void setHeadPoint(PointF headPoint) {
        ...
        middlePoint = calculatePoint(headPoint, BODY_LENGTH, fishMainAngle + 180);
    }

    /**
     * @param start 鱼头中点坐标
     */
    public Fish(PointF start) {
        init(start);
    }
    ...
    private void init(PointF start) {
        mPaint = new Paint();
        mPath = new Path();
...
//        middlePoint = new PointF(4.19f * HEAD_RADIUS, 4.19f * HEAD_RADIUS);
    ...
    public void draw(@NonNull Canvas canvas) {
        //鱼的初始角度  sin 变动更加平滑 -1 - 1 ； 所以鱼的头的角度变动幅度 为 -5 - 5 ；
        ...
        PointF smallCenterPoint = makeSegment(canvas, bodyBottomCenterPoint, BIG_CIRCLE_RADIUS, MIDDLE_CIRCLE_RADIUS, FIND_MIDDLE_CIRCLE_LENGTH,
                fishAngle, true);
        canvas.drawCircle(smallCenterPoint.x, smallCenterPoint.y, MIDDLE_CIRCLE_RADIUS, mPaint);
        //画小节支
        ...
        //画身体
        makeBody(canvas, headPoint, fishAngle, bodyBottomCenterPoint);
        //画眼睛
     ...
     * @param fishAngle
     */
    private void makeBody(Canvas canvas, PointF headPoint, float fishAngle, PointF bottomCenterPoint) {
