...
public class ElectricFanLoadingRenderer extends LoadingRenderer {
    ...
    ElectricFanLoadingRenderer(Context context) {
        super(context);
        init();
        setupPaint();
    }
    ...
    private void init() {
        ...
        mWidth = ExtensionHelperKt.getDpToPx(DEFAULT_WIDTH);
        mHeight = ExtensionHelperKt.getDpToPx(DEFAULT_HEIGHT);
        mTextSize = ExtensionHelperKt.getDpToPx(DEFAULT_TEXT_SIZE);
        mStrokeWidth = ExtensionHelperKt.getDpToPx(DEFAULT_STROKE_WIDTH);
        mCenterRadius = ExtensionHelperKt.getDpToPx(DEFAULT_CENTER_RADIUS);
        mProgressCenterRadius = ExtensionHelperKt.getDpToPx(DEFAULT_PROGRESS_CENTER_RADIUS);
        ...
        final Resources resources = AppUtil.INSTANCE.getApplication().getResources();
        mLeafDrawable = ResourcesCompat.getDrawable(resources, R.drawable.ic_leaf, null);
        mLoadingDrawable = ResourcesCompat.getDrawable(resources, R.drawable.ic_loading, null);
        mElectricFanDrawable = ResourcesCompat.getDrawable(resources, R.drawable.ic_eletric_fan, null);
     ...
     */
    @Override
    protected void computeRender(float p) {
        float renderProgress = p / 100;
