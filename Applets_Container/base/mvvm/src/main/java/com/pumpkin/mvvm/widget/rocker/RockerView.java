package com.pumpkin.mvvm.widget.rocker;
...
public class RockerView extends View {
    ...
    protected void init() {
        // 可移动区域背景
        mAreaColor = UIHelper.INSTANCE.getThemeAttrColor(getContext(), R.attr.colorOnSurfaceVariant);
        mAreaAlpha = 80;
        // 摇杆背景
        mRockerColor = UIHelper.INSTANCE.getThemeAttrColor(getContext(), R.attr.colorPrimary);
        mRockerAlpha = 100;
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        // 摇杆半径 是可滑动区域半径的 2/5
        mRockerRadius = measureWidth / 2 / 5 * 2;
        if (BuildConfig.DEBUG) {
            Log.d(TAG, "onMeasure: --------------------------------------");
            Log.d(TAG, "onMeasure: widthMeasureSpec = " + widthMeasureSpec + " heightMeasureSpec = " + heightMeasureSpec);
            Log.d(TAG, "onMeasure: widthMode = " + widthMode + "  measureWidth = " + widthSize);
            Log.d(TAG, "onMeasure: heightMode = " + heightMode + "  measureHeight = " + widthSize);
            Log.d(TAG, "onMeasure: measureWidth = " + measureWidth + " measureHeight = " + measureHeight);
        }
     ...
     * 返回参数
     */
    private void callBack(double radian) {
        if (null != mOnAngleChangeListener) {
